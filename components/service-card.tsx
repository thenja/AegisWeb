...
export function ServiceCard({ title, summary, href }: ServiceCardProps) {
  return (
    <Link
      href={href}
      className="group relative block h-full min-h-[148px] overflow-hidden rounded-xl border border-slate-200 bg-white/95 px-3.5 py-3 shadow-panel transition duration-200 hover:-translate-y-0.5 hover:border-brand-400 hover:shadow-md focus-visible:outline-none sm:min-h-[160px] sm:px-5 sm:py-3.5"
    ...
    >
      ...
      />
      <h3 className="pr-8 text-[0.98rem] font-semibold font-[var(--font-body)] text-accent-deep sm:text-base">{title}</h3>
      <p className="mt-1.5 pr-8 text-[0.9rem] leading-relaxed text-slate-700 sm:text-sm">{summary}</p>
      <span
        aria-hidden="true"
